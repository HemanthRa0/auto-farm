 ...
 // const db = getDatabase(app);
  ...
  useEffect(() => {
    if (data.length === 0)
      get(child(dbref, "Home/"))
        ...
        .then((data) => {setData(data);
          ...
          setDet(data["Detail"])
          setTagL(data["Title"])
          setSubt(data["Subtitle"])
          setGtext(data["GreenText"])
    ...
    async function Push() {
      ...
      if(newfile.length != 0){
      const storageRef = sref(storage, "Home/"+newfile[0].name);
    await uploadBytes(storageRef, newfile[0]).then((snapshot) => {
        console.log("Image Uploaded");
      });
      await getDownloadURL(sref(storage, "Home/"+newfile[0].name))
      .then((url) => {
        setImgurl(url)
  ...
  return( 
  <div className=""><h1 className="text-green-800 font-medium text-center text-4xl ">Home Page</h1>
    ...
    <form
      ...
      >
        <div className=" md:w-full sm:w-full h-auto w-auto md:mr-12">
          ...
          /> 
          <h1 className="text-xl font-bold p-2">Subtitle :</h1>
              <div className="ml-2">
              <textarea
            value={subt}
          ...
          /></div>
          <h1 className="text-xl font-bold p-2">Green Text :</h1>
              <div className="ml-2">
              <textarea
            value={gtext}
            className="w-full h-72 border-2 border-golden p-4 bg-gray-200"
            ...
            placeholder="Enter the text here"
          /></div>
          <h1 className="text-xl font-bold p-2">Details :</h1>
              <div className="ml-2">
              <textarea
            value={det}
            className="w-full h-72 border-2 border-golden p-4 bg-gray-200"

 ...
 // const db = getDatabase(app);
  ...
  useEffect(() => {
    ...
    async function Push() {
      ...
      if(newfile.length != 0){
      const storageRef = sref(storage, "Info/"+newfile[0].name);
    await uploadBytes(storageRef, newfile[0]).then((snapshot) => {
        console.log("Image Uploaded");
      });
      await getDownloadURL(sref(storage, "Info/"+newfile[0].name))
      .then((url) => {
        setImgurl(url)
  ...
  return( 
  <><h1 className="text-green-800 font-medium text-center text-4xl ">Our Story</h1>

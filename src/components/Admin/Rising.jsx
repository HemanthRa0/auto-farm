 ...
 // const db = getDatabase(app);
  ...
  useEffect(() => {
    ...
    async function Push() {
      ...
      if(newfile.length != 0){
      const storageRef = sref(storage, "Rising/"+newfile[0].name);
    await uploadBytes(storageRef, newfile[0]).then((snapshot) => {
        console.log("Image Uploaded");
      });
      await getDownloadURL(sref(storage, "Rising/"+newfile[0].name))
      .then((url) => {
        setImgurl(url)
  ...
  return( 
  <><h1 className="text-green-800 font-medium text-center text-4xl ">Rising Page</h1>
